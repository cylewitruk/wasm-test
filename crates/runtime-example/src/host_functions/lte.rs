...
#[allow(dead_code)]
fn fn_less_than_or_equal_to(mut store: impl AsContextMut<Data = ClarityWasmContext>) -> Func {
    Func::wrap(
        &mut store,
        #[inline]
        |caller: Caller<'_, ClarityWasmContext>, a_ptr: i32, b_ptr: i32| -> i32 {
            caller.as_stack().exec(|frame: StackFrame<'_>| {
                ...
                let result = match (a, b) {
                    ...
                    _ => unimplemented!("Less-than-or-equal-to is not implemented for given types"),
                };
